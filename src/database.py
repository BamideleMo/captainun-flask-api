...
class Country(db.Model):
    ...
    climate_goal=db.Column(db.Text,nullable=False)
...
class Updates(db.Model):
    id=db.Column(db.Integer, primary_key=True)
    update_for=db.Column(db.Integer,nullable=False)
...
class Ratings(db.Model):
    ...
    rating_for=db.Column(db.Text,nullable=False)
    rating_by=db.Column(db.Text,nullable=False)
    created_at = db.Column(db.String(120), default=(datetime.now().strftime("%d.%m.%Y")))
    updated_at = db.Column(db.String(120), onupdate=(datetime.now().strftime("%d.%m.%Y")))
